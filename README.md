...
| 項目 | 狀態 |
|---|---|
| 規格書 | ✅ v2.0 完成（含 2026-08-21 / 2026-08-25 / 2026-08-26 / 2026-08-27 / 2026-08-28 九輪實作回饋修訂，見 [00 §0.7–§0.15](docs/spec/00-master.md)） |
| `environment/` | ✅ Phase 2 完成：唯一 compose 檔、雙 Dockerfile、四環境樣板、8 支腳本（含 90 項 DoD gate 的 `dod.sh`）、CI compose 驗證。Phase 13 修正樣板 `JWT_SECRET`——原值 `CHANGE_ME_MIN_32_BYTES` 自己只有 22 bytes，HS256 上線後會讓照樣板複製的全新環境啟動失敗 |
...
| 架構決策 | [`docs/architecture/decisions/`](docs/architecture/decisions/)（ADR 0001–0013：各 phase 的規格衝突處置、實作決策、環境維護、M1 總複查、Phase 13 認證層決策與其收尾稽核修正） |
