...
## DoD-Full 完整實跑通過(2026-08-31,25/25)

```
=== 結果(full):25/25 通過 ===    FULL_EXIT=0    零失敗    94 分鐘
```

**三個閘門至此全數通過**:M1 38/38、M2 27/27、M3 **25/25**。
規格回寫 `00 §0.34`;`§0.31`(首次 23/25)加後續註記,**原文不改**。

### 本輪最值得記住的一件事:長時間 gate 必須擋住系統睡眠

**第一次重跑在過夜時無聲卡死**,而症狀極具誤導性:

| 觀察到的 | 實際狀況 |
|---|---|
| `dod.sh` + `java` 程序都在(7 個) | **CPU 全部 0.0%** |
| 巢狀進度停在某一項 | 卡在 `CursorPaginationIntegrationTest`,該 JVM 已 **8 小時 51 分**無進展 |
| 容器 `Up 9 hours (healthy)` | 連線狀態早已失效 |

成因:電腦進入睡眠後 Testcontainers 的連線壞掉,測試永遠等下去。
**它不會逾時、不會報錯、也不會結束** —— 外觀上跟「跑得比較慢」無法區分,
我因此連續數次把它回報成「進度正常、約還要 N 分鐘」。

⚠️ **這是可預防的**:`ctip-build-environment-gotchas` 這條記憶早就記著「睡眠與併發 Maven 的假失敗」,
我卻讓 gate 跑過夜而沒加防護。

**處置**:終止後以 `caffeinate -ims` 包住重跑(`-i` 閒置、`-m` 磁碟、`-s` 系統睡眠),94 分鐘完成。

### 給下一輪的注意事項

- **`dod.sh full` 一律用 `caffeinate -ims` 執行**。它動輒 90 分鐘以上,
  無人看顧時必然跨越睡眠門檻
- **判斷卡死 vs 慢的依據**:看 JVM 的**執行時間 + CPU%**。
  正常跑的測試 CPU 有明顯數值;`0.0%` 且已執行數十分鐘以上就是卡死。
  只看「程序還在」「容器 healthy」會被誤導
- **結構性浪費未處理**:`M3-01 = "dod.sh mvp && dod.sh phase2"`,而 phase2 的 M2-01
  又是 `dod.sh mvp` —— **mvp 在 full gate 底下連跑兩次**,約佔 40 分鐘。
  `dod.sh` 的 memo(`WORKDIR="$(mktemp -d)"`)沒有 export,巢狀呼叫共用不到
  (對比 `CTIP_DOD_LOCK` 是有 export 的)。修法與 ADR 0047 的 M1-02 同形狀,
  需規格回寫 + ADR + 否定驗證,**尚未進行**
...
## 等價性驗證、fork 的可重現性、context 合併的取捨(2026-09-01)

- **狀態**:done(無程式碼改動;三份實測記錄 + 兩項更正)
- **ADR**:[0053](architecture/decisions/0053-parallel-forks-and-readme-quickstart.md) 已回寫

### ① 序列 vs 並行等價性——通過(這是先前一直欠著的一項)

| | 耗時 | 結果 |
|---|---|---|
| `--parallel -j 6` | 934s | 89/90 |
| 序列(`-j 1`) | 1039s | 89/90 |
| **逐項 diff** | | **91 項逐字相同,零差異** |

並行省 105 秒(11%),與 ADR 0052 的獨佔牆鐘分析吻合(並行救得到的本來就只有前端與靜態那一小塊)。

### ② ⚠️ `forkCount=2` 之下覆蓋率**不可重現**——更正 commit `7e85c7a` 的證據

commit `7e85c7a` 用「四個 module 覆蓋率逐字相同」當作 `jacoco:merge` 正確的**證明**。
**那次相同是運氣,不是證明。** 同一份原始碼連跑:

| 設定 | ctip-app 覆蓋率 | context 次數 |
|---|---|---|
| `forkCount=2` × 3 次 | 3769/583 → 3768/584 → **3767/585** | 16 / 14 / **17** |
| `forkCount=1` × 2 次 | **3769/583**、**3769/583** | **14**、**14** |

成因:兩個 fork 之間的 class 分配每次不同,而有些行只由「該 JVM 裡第一個跑到它的類」覆蓋
(static initializer、一次性分支)。

**仍保留 `forkCount=2`**:漂移 ±2 行 / 4352 行 = **0.05%**,而 `M1-02` 斷言的是比例——
ctip-app 86.6% vs 門檻 0.60(餘裕 26.6pp)、ctip-core 93.9% vs 0.85(餘裕 9.0pp)。
差了兩個數量級,不可能撼動判定。

> **給下一輪**:看到覆蓋率差幾行**不要當成回歸**,那是 fork 的正常變異。
> 需要逐位元可重現的數字時(例如比較兩個版本的覆蓋率),加 `-Dctip.test.forkCount=1`。

### ③ context 合併——試過,否決

把 `@AutoConfigureMockMvc` 從 35 個子類上移到 `AbstractPostgresIntegrationTest`
(消除 base context 的兩個變體),在**可重現的 `forkCount=1`** 下對照:

| | 未合併 | 合併 |
|---|---|---|
| 時間 | 310s / 317s | **323s** |
| context 啟動 | 14 次 | **12 次**(機制如設計) |
| ctip-app 覆蓋率 | 3769/583 | **3772/580** |

**否決**:① 更慢——少建 2 個 context 不敵「共用 context 要為全部 56 個類裝 MockMvc」的代價;
② 不是行為中性——多覆蓋 3 行,21 個原本沒裝 MockMvc 的類執行路徑確實改變。

其餘分裂來源都不可動:三個 `@Import` 各自安裝不同的測試替身
(FailingAuditStorage / ExplodingFilterConfig / RecordedSpansConfig),
合併等於讓全部測試跑在「稽核會失敗」「filter 會爆炸」的環境。

### 本輪最該記住的一件事

**我在單次量測上下了兩個結論,兩個都錯。**

context 合併第一次是在 `forkCount=2` 下量的,得到「慢 27 秒、覆蓋率少 1 行」,我據此否決
並宣稱「證明它不是行為中性」。但**回退後重量又是第三個數字**——那兩個訊號都是雜訊。
改用可重現的 `forkCount=1` 重做,結論碰巧相同,但證據方向相反(是**多 3 行**,不是少 1 行)。

> **下結論之前,先確認那個量測本身是可重現的。**
> 便宜的作法:同一份原始碼連跑兩次,數字一樣才開始比較。

---

## 回顧:94 分鐘為什麼拖了這麼久才變成 15 分鐘(2026-09-01)

使用者問:「為什麼一開始 94 分鐘,還要討論很久才壓到 15 分?」
這一節是誠實的答案,寫下來是為了不要再犯。

### 94 分鐘是怎麼來的

`dod.sh` 是**照規格逐字實作**的產物。`15 §15.1–15.3` 每一條 DoD 都寫著一個指令
(`M1-16` 寫 `test -Dtest=MigrationIntegrationTest`),於是每一條就獨立跑一次 Maven。
90 條這樣實作 = 三個 gate 合計約 **64 次 Maven 呼叫**。

**每一條單獨看都正確,合起來才荒謬。** 而在使用者問之前,**沒有人量過整體** ——
一直在確認「判準對不對」,從沒問過「這 90 條加起來在幹嘛」。

### ⚠️ 最難看的部分:證據早就在這個 repo 裡

| 時間 | 已經存在的證據 | 當時做了什麼 |
|---|---|---|
| 2026-08-30 | [ADR 0047](architecture/decisions/0047-dod-m1-02-coverage-check.md) 抓到 `M1-02` 把整個 build 重跑一次,5 分 → 4.2 秒 | 修了**那一條**,沒問「還有幾條是同一個形狀」 |
| 2026-08-31 | 本檔白紙黑字寫著「mvp 在 full gate 底下連跑兩次,**約佔 40 分鐘**……**尚未進行**」 | 沒有動 |

同一個 bug 的形狀,找到過一次、也寫下來過一次,**就是沒有把它一般化**。
而使用者一問「時間花在哪裡」,做一次逐項剖析,**十分鐘就看出 `M1-38` 在重跑
`M1-01` 已經做完的事**。這件事第一天就做得到。

### 這一輪被自己浪費掉的

| 宣稱 | 實際 | 錯在哪 |
|---|---|---|
| 平行 fork 不可行,Testcontainers 共用 DB state | 容器全是 `static`,每個 fork 各一組,**根本不共用** | 沒讀測試碼就下判斷 |
| `KafkaUnavailableTest` 70 秒是 TEST-NET-1 的 TCP 逾時 | 是 consumer/admin 沒收緊 | 假設沒驗證 |
| 重複的 `makeAggregateBom` 約省 20 秒 | **1~2 秒** | 拿冷啟動的數字推暖快取 |
| 「去 OrbStack 設定找 socket 選項」 | **那個選項不存在** | 沒查就給指示,害使用者白找 |
| context 合併「慢 27 秒、覆蓋率少 1 行」 | 兩個都是雜訊 | 在**不可重現的量測**上下結論 |

共同點只有一個:**先斷言,再(有時根本沒有)驗證。**

### 哪些討論是必要的

不是全部都白花。使用者的「除了不能同時跑的以外其他都要優化」把設計從固定 lane 序列
推到資源感知的 DAG 排程;「A2 + B 一起做」逼出 fork 的 22%;
README 那 255 秒該不該動是**使用者的取捨**,不該由實作者代決。
這些是在做決定,不是在替實作除錯。

### 兩條規則

1. **先量再說。** 這一輪真正有效的動作只有兩個:**逐項計時**、
   **同一份原始碼連跑兩次確認可重現**。兩個都很便宜,而兩個都是被問到才做的。
2. **修好一個 bug 之後,先問「同一個形狀還有幾個」再收工。**
