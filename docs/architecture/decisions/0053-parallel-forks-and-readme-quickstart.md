...
| `M1-01` | 334s | **245s** |
| `M1-38` | 345s | **136s** |
| `dod.sh full` 整輪 | 1137s(18:57) | **1009s(16:49)** |

整輪只少了 128 秒,比兩項相加(−298s)少——因為這一輪的 `M1-14` 從 90s 變成 218s:
**`backend/pom.xml` 一改,`up.sh` 的相依漂移守衛就判定快取需要重新預熱**(這正是它該做的事)。
那是一次性成本;改動後再量,兩個守衛分別是 16 秒與 1 秒,不會再預熱。
扣掉這一次性的 128 秒與 heavy 批次修正的 19 秒,**穩態預估約 14 分半**。
