...
> ✅ **後續（2026-08-31，§0.34）**：兩項皆已修復並**完整重新實跑，結果 25/25、零失敗**。
> 本節原文保留當時的實況不修改——「23/25 → 修了什麼 → 25/25」這條線本身就是紀錄的價值。
兩項往下追共發現三件事；完整記錄見
...
## 0.34 DoD-Full 完整實跑通過（2026-08-31，25/25）

`./environment/scripts/dod.sh full` **25/25 通過、零失敗**，用時 94 分鐘。
**三個閘門至此全數通過**：M1 38/38、M2 27/27、M3 **25/25**。

§0.31 記錄的首次實跑為 23/25（失敗 M3-01、M3-19），兩項的修復分別在
[§0.32](#032-實作回饋修訂2026-08-30全專案複查後回寫)、
[ADR 0048](../architecture/decisions/0048-ci-green-and-test-isolation.md)、
[ADR 0049](../architecture/decisions/0049-base-image-vulnerability-remediation.md)。
本次是**修復後對當前程式碼的完整驗證**，不是重述先前的單項結果。

### 一個過程中的教訓：長時間 gate 必須擋住系統睡眠

**第一次重跑在過夜時無聲卡死。** 症狀極具誤導性：

- `dod.sh` 與 `java` 程序都還在（7 個），**但 CPU 全部 0.0%**
- 卡在 `CursorPaginationIntegrationTest`，該 JVM 持續 **8 小時 51 分**毫無進展
- 容器仍顯示 `Up 9 hours (healthy)`

成因是電腦進入睡眠後 Testcontainers 的連線狀態失效，測試就永遠等下去。
**它不會逾時、不會報錯、也不會結束**——外觀上跟「跑得比較慢」無法區分。

處置：終止後以 `caffeinate -ims` 包住重跑（`-i` 閒置、`-m` 磁碟、`-s` 系統睡眠），
94 分鐘完成。**`dod.sh full` 動輒 90 分鐘以上，往後一律以 `caffeinate` 執行。**

> 判斷是否卡死的依據：看 **JVM 的執行時間 + CPU%**。
> 正常運作的測試 CPU 會有明顯數值；`0.0%` 且已執行數十分鐘以上即為卡死，不是慢。

---

*主綱結束。規格版本 v2.0（含 2026-08-21、2026-08-25、2026-08-26、2026-08-27、2026-08-28、2026-08-29、2026-08-30、2026-08-31 實作回饋修訂，見 §0.7–§0.34）。*
